...
type FreeagentAccessToken struct {
...
func GetAccessToken() (string, error) {
	...
	result, err := ssmClient.GetParameters(&ssm.GetParametersInput{
		Names: aws.StringSlice([]string{
			"/control_alt_repeat/freeagent/live/access_token",
			"/control_alt_repeat/freeagent/live/expires_in",
		}),
	...
	for _, param := range result.Parameters {
		if *param.Name == "/control_alt_repeat/freeagent/live/access_token" {
			access_token = *param.Value
			access_token_last_modified = *param.LastModifiedDate
		}
		if *param.Name == "/control_alt_repeat/freeagent/live/expires_in" {
